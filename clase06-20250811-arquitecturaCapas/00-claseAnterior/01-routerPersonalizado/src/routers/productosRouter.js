...
export const productosRouter = new CustomRouter()
...
productosRouter.get(
    ...
    (req, res) => {
        ...
        if(req.query.error){
            res.badRequest(`Error de argumentos inválidos...!!!`)
        }

        // res.setHeader('Content-Type', 'application/json');
        // return res.status(200).json({ payload: productos });
        res.success("Listado productos", productos, 200)
    }
)

productosRouter.get("/datos", (req, res)=>{

    if(req.query.error){
        throw new Error(`Error de argumentos...!!!`)
    }    

    res.success("datos productos")
})